...
CREATE TABLE public.geo_attendance_rooms (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  section text,
  range_meters integer NOT NULL DEFAULT 30 CHECK (range_meters >= 1 AND range_meters <= 500),
  duration_minutes integer NOT NULL DEFAULT 50 CHECK (duration_minutes >= 1 AND duration_minutes <= 600),
...
CREATE TABLE public.staffs (
  user_id uuid NOT NULL,
  staff_uid text NOT NULL DEFAULT ('S-'::text || substr(replace((gen_random_uuid())::text, '-'::text, ''::text), 1, 10)) UNIQUE,
  full_name text NOT NULL,
  phone text,
  designation text NOT NULL DEFAULT 'Administrative Staff'::text,
  department text NOT NULL DEFAULT 'CSE'::text,
  is_admin boolean NOT NULL DEFAULT false,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT staffs_pkey PRIMARY KEY (user_id),
  CONSTRAINT staffs_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(user_id)
);
CREATE TABLE public.geo_attendance_codes (
  room_id uuid NOT NULL,
  code text NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT geo_attendance_codes_pkey PRIMARY KEY (room_id),
  CONSTRAINT geo_attendance_codes_room_fkey FOREIGN KEY (room_id) REFERENCES public.geo_attendance_rooms(id)
);
CREATE TABLE public.admin_direct_bookings (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  room_number text NOT NULL,
  booking_date date NOT NULL,
  day_of_week integer NOT NULL CHECK (day_of_week >= 0 AND day_of_week <= 6),
  start_time time without time zone NOT NULL,
  end_time time without time zone NOT NULL,
  booking_type text NOT NULL DEFAULT 'periodic'::text CHECK (booking_type = ANY (ARRAY['periodic'::text, 'continuous'::text])),
  label text NOT NULL DEFAULT 'Reserved'::text,
  booked_by_user_id uuid NOT NULL,
  status text NOT NULL DEFAULT 'approved'::text CHECK (status = ANY (ARRAY['approved'::text, 'cancelled'::text])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.routine_generation_jobs (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  session text NOT NULL,
  ...
  CONSTRAINT routine_generation_jobs_requested_by_fkey FOREIGN KEY (requested_by) REFERENCES public.profiles(user_id)
);
CREATE TABLE public.routine_drafts (
  ...
  CONSTRAINT routine_drafts_job_id_fkey FOREIGN KEY (job_id) REFERENCES public.routine_generation_jobs(id)
);
CREATE TABLE public.routine_draft_slots (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  conflict_status text NOT NULL DEFAULT 'valid'::text CHECK (conflict_status = ANY (ARRAY['valid'::text, 'warning'::text, 'conflict'::text])),
  conflict_reasons jsonb NOT NULL DEFAULT '[]'::jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT routine_draft_slots_pkey PRIMARY KEY (id),
  CONSTRAINT routine_draft_slots_draft_id_fkey FOREIGN KEY (draft_id) REFERENCES public.routine_drafts(id),
  CONSTRAINT routine_draft_slots_course_offering_id_fkey FOREIGN KEY (course_offering_id) REFERENCES public.course_offerings(id),
  CONSTRAINT routine_draft_slots_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(id),
  CONSTRAINT routine_draft_slots_teacher_user_id_fkey FOREIGN KEY (teacher_user_id) REFERENCES public.teachers(user_id),
  CONSTRAINT routine_draft_slots_room_number_fkey FOREIGN KEY (room_number) REFERENCES public.rooms(room_number)
);
CREATE TABLE public.teacher_availability (
  ...
  start_period integer NOT NULL,
  end_period integer NOT NULL,
  availability_type text NOT NULL CHECK (availability_type = ANY (ARRAY['available'::text, 'unavailable'::text, 'preferred'::text, 'not_preferred'::text])),
  priority integer NOT NULL DEFAULT 1,
  note text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT teacher_availability_pkey PRIMARY KEY (id),
  CONSTRAINT teacher_availability_teacher_user_id_fkey FOREIGN KEY (teacher_user_id) REFERENCES public.teachers(user_id)
);
CREATE TABLE public.course_schedule_requirements (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  session text NOT NULL,
  year integer NOT NULL,
  term integer NOT NULL,
  ...
  lab_groups ARRAY NOT NULL DEFAULT '{}'::text[],
  preferred_room_type text,
  preferred_room_numbers ARRAY NOT NULL DEFAULT '{}'::text[],
  priority integer NOT NULL DEFAULT 1,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT course_schedule_requirements_pkey PRIMARY KEY (id),
  CONSTRAINT course_schedule_requirements_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(id),
  CONSTRAINT course_schedule_requirements_course_offering_id_fkey FOREIGN KEY (course_offering_id) REFERENCES public.course_offerings(id)
);
CREATE TABLE public.routine_constraints (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  constraint_key text NOT NULL UNIQUE,
  constraint_type text NOT NULL CHECK (constraint_type = ANY (ARRAY['hard'::text, 'soft'::text])),
...
CREATE TABLE public.routine_versions (
  ...
  CONSTRAINT routine_versions_published_by_fkey FOREIGN KEY (published_by) REFERENCES public.profiles(user_id)
);