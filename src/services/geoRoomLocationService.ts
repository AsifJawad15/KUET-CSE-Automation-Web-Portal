...
export async function addGeoRoomLocation(
  input: AddGeoRoomInput,
): Promise<ServiceResult<GeoRoomLocation>> {
  return apiClient.post<GeoRoomLocation>(ENDPOINT, input);
}

export async function updateGeoRoomLocation(
  id: string,
  input: AddGeoRoomInput,
): Promise<ServiceResult<GeoRoomLocation>> {
  return apiClient.patch<GeoRoomLocation>(ENDPOINT, { id, ...input });
}
